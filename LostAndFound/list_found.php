...
if($user_role == 'admin' && isset($_POST['update_status'])) {
    ...
    if(mysqli_query($connect, $update_sql)) {
        $message = "Item status updated successfully to: " . ucfirst($new_status) . "!";
        // Auto refresh untuk lihat perubahan
...
<head>
    ...
    <style>
        ...
        .dot-approved { background: #0d6efd; }
        .dot-matched { background: #17a2b8; }
        .dot-claimed { background: #28a745; }
            ...
            <?php if(mysqli_num_rows($result) > 0): ?>
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                        ...
                        <tbody>
                            ...
                            <tr style="border-bottom: 1px solid #eee; transition: all 0.3s;" onmouseover="this.style.backgroundColor='#f9f9f9'" onmouseout="this.style.backgroundColor='white'">
                                <!-- Type with Status Dot -->
                                <td style="padding: 15px; color: #333;">
                                    <div style="display: flex; align-items: center; gap: 10px;">
                                        <span class="status-dot dot-<?php echo $item['status']; ?>"></span>
                                    ...
                                    <td style="padding: 15px;">
                                        <form method="POST" action="" style="display: flex; gap: 5px; align-items: center;">
                                            ...
                                            <select name="status" style="padding: 8px; border: 1px solid #ddd; border-radius: 5px; 
                                                                        font-size: 14px; background: white; min-width: 120px;" required>
                ...
                <div style="margin-top: 30px; padding: 20px; background: #f8f9fa; border-radius: 10px;">
                    <h4 style="color: #2c3e50; margin-bottom: 15px; font-size: 16px;">
                        <i class="fas fa-info-circle" style="color: #27ae60; margin-right: 8px;"></i>
                        Status Information
                    </h4>
                    <div style="display: flex; flex-wrap: wrap; gap: 15px;">
                        <div style="display: flex; align-items: center; gap: 8px;">
                            <span class="status-dot dot-pending"></span>
                            <span class="status-badge status-pending" style="margin-right: 5px;">Pending</span>
                            <small style="color: #666;">Item baru dilaporkan</small>
                        </div>
                        <div style="display: flex; align-items: center; gap: 8px;">
                            <span class="status-dot dot-approved"></span>
                            <span class="status-badge status-approved" style="margin-right: 5px;">Approved</span>
                            <small style="color: #666;">Disahkan oleh admin</small>
                        </div>
                        <div style="display: flex; align-items: center; gap: 8px;">
                            <span class="status-dot dot-matched"></span>
                            <span class="status-badge status-matched" style="margin-right: 5px;">Matched</span>
                            <small style="color: #666;">Dijumpai pemilik yang sesuai</small>
                        </div>
                        <div style="display: flex; align-items: center; gap: 8px;">
                            <span class="status-dot dot-claimed"></span>
                            <span class="status-badge status-claimed" style="margin-right: 5px;">Claimed</span>
                            <small style="color: #666;">Sudah diambil oleh pemilik</small>
                        </div>
                        <div style="display: flex; align-items: center; gap: 8px;">
                            <span class="status-dot dot-closed"></span>
                            <span class="status-badge status-closed" style="margin-right: 5px;">Closed</span>
                            <small style="color: #666;">Kes sudah selesai</small>
                        </div>
                    </div>
                </div>
            ...
            <div style="margin-top: 40px; padding: 20px; background: #e8f8ef; 
                ...
                <p style="color: #666; line-height: 1.6; font-size: 14px;">
                    <?php if($user_role == 'admin'): ?>
                        ...
                        • Click "View" to see complete details or "Edit" to modify item information.<br>
                        • <strong>Status Flow:</strong> Pending → Approved → Matched → Claimed → Closed
                    <?php else: ?>
